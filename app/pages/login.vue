  ...
  <UiAuthLayout :sub-title="t('auth.loginToNGS')" height-img="h-[70vh]!">
    <UForm ref="formRef" :schema :state="formLogin" class="space-y-6 max-sm:space-y-4">
      ...
      <div>
        ...
        <p
          class="text-primary hover:cursor-pointer hover:underline text-sm text-end mt-2 font-medium"
    ...
    <p class="text-sm font-medium text-center mt-4">
      {{ t('auth.haveNotAccount') }}
      <span class="text-primary hover:cursor-pointer hover:underline font-bold text-sm" @click="navigateTo('/register')">{{
        t('auth.register')
      }}</span>
    </p>
